    ...
    app.get('/', (req, res) => {
        ...
        //  console.log('Session:', req.session);
         ...
         if (req.session.views) {
             req.session.views++;
             res.send(`<h2> Hi There , my name is Mary Jane . You've visited ${req.session.views} times.</h2>`);
         } else {
             req.session.views = 1;
             res.send('<h2> Hi There , my name is Mary Jane. Welcome for the first time!</h2>');
         }
    });
